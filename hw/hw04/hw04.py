...
def balanced(m):
    ...
    if is_planet(m):
        ...
        assert is_mobile(m)
        if (total_mass(end(left(m)))*length(left(m)) == 
            total_mass(end(right(m))) * length(right(m))) and balanced(end(left(m))) and balanced(end(right(m))):
            return True
        else:
            return False
...
def max_path_sum(t):
    ...
    if is_leaf(t):
        return label(t)
    branch_sums = [max_path_sum(branch) for branch in branches(t)]
    return label(t) + max(branch_sums)
