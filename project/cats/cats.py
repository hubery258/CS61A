...
def about(subject):
    ...
    "*** YOUR CODE HERE ***"
    def check(paragraph):
        content = split(lower(remove_punctuation(paragraph)))
        for word in subject:
            if word in content:
                return True
        return False
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    min_diff , min_dif_word = diff_function(typed_word, word_list[0], limit), word_list[0]
    for word in word_list:
        if diff_function(typed_word, word, limit) < min_diff:
            min_diff, min_dif_word = diff_function(typed_word, word, limit), word
    if min_diff > limit:
        return typed_word
    return min_dif_word
    # END PROBLEM 5
...
def feline_fixes(typed, source, limit):
    ...
    """
    # BEGIN PROBLEM 6
    gap = abs(len(typed) - len(source))
    if gap > limit:
        return limit + 1
    elif limit == 0 and typed[0] != source[0]:
        return 1
...
def minimum_mewtations(typed, source, limit):
    ...
    if gap > limit:
        return limit + 1
    # 先把逻辑改了 in就ok
    elif len(typed) == 1 or len(source) == 1:
        if typed[0] in source or source[0] in typed:
            return gap
        else:
            return 1 + gap
    # return min(...) 三个可能性,我只对第一位分析,基层情况的单if太多了，理一下
    ...
    if typed[0] == source[0]: # Feel free to remove or add additional cases
        ...
        # BEGIN
        "*** YOUR CODE HERE ***"
        return 1 + min(minimum_mewtations(add, source, limit - 1)
                   ,minimum_mewtations(remove, source, limit - 1)
                   ,minimum_mewtations(substitute, source, limit - 1))
        # END
...
def report_progress(typed, source, user_id, upload):
    ...
    for i in range(min(len(source),len(typed))):
        if i == min(len(source),len(typed))-1 and typed[i] == source[i]:
            i += 1
            break
        if typed[i] != source[i]:
            break
...
def time_per_word(words, timestamps_per_player):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    times = [ [ player[time_num] - player[time_num - 1] 
               for time_num in range(1, len(player))] 
...
def fastest_words(match):
    ...
    for word_index in word_indices:
        min_player, min_time = 0 , time(match, 0, word_index)
        for player_index in player_indices:
            if time(match, player_index, word_index) < min_time:
                min_player , min_time = player_index, time(match, player_index, word_index)
        result[min_player] = result[min_player] + [get_word(match, word_index)]
    return result 
    # END PROBLEM 10
