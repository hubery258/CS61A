...
>>> def pressure(v, t, n=6.022e23):
        ...
        return n * k * t / v
```
当只提供两个函数时自动把n处理为默认值，`n=...`这里没有进行赋值操作而是表示调用`pressure`函数时使用的默认值。注意，`help(xx)`在使用之前需要保证解释器读取得到这个函数，比如xx在hw01文件夹下的某个python文件内，在这个文件夹内打开解释器，那么要先import hw01让解释器的环境里存在hw01，之后再执行`help(hw01.xx)`.
