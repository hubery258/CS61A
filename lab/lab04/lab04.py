def divide(quotients, divisors):
    ...
    """
    return {q: [ num for num in divisors if num % q == 0] for q in quotients}
...
def buy(required_fruits, prices, total_amount):
    ...
    [9 apples][1 kiwi]
    """
    count = {fruit: 0  for fruit in required_fruits}
    def add(fruits, amount):
        if fruits == [] and amount == 0:
            result = ''
            for fruit in count.keys():
                result += display(fruit, count[fruit])
            print(result)
...
def closer_city(lat, lon, city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    location = make_city('start', lat, lon)
